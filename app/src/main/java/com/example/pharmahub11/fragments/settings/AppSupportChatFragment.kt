...
import com.example.pharmahub11.databinding.FragmentAppSupportChatBinding
...
class AppSupportChatFragment : Fragment() {
    private lateinit var binding: FragmentAppSupportChatBinding
    private lateinit var adapter: MessageAdapter
    private val messages = mutableListOf<Message>()
    private var chatId: String = ""

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = FragmentAppSupportChatBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun checkExistingChatOrCreateNew() {
        ...
        FirebaseFirestore.getInstance()
            ...
            .whereEqualTo("chatType", "app_support")
            .limit(1)
            .get()
            .addOnSuccessListener { querySnapshot ->
                if (querySnapshot.isEmpty) {
                    createNewAppSupportChat(currentUser)
                } else {
    ...
    private fun createNewAppSupportChat(currentUser: FirebaseUser) {
        chatId = UUID.randomUUID().toString()
        val chat = Chat(
            chatId = chatId,
            customerId = currentUser.uid,
            customerName = currentUser.displayName ?: "Customer",
            chatType = "app_support"
        )
